...
func BenchmarkPutInt_orcacheLRU2(b *testing.B) {
	cache := orcache.NewLRUCache(256, 16, 10*time.Second).LRU2(16)
	for i := 0; i < b.N; i++ {
...
func BenchmarkHeavyRead_orcacheLRU2(b *testing.B) {
	gocache.GCPause()
...
func BenchmarkHeavyWrite_orcacheLRU2(b *testing.B) {
	gocache.GCPause()
